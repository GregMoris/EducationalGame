...
let stopwatch = document.getElementById("clock"), //Получение эллемента секундомера по идентификатору clock из fg_level1_index.html
    stopwatchContent = stopwatch.textContent, // Присвоение текстового содержимого переменной
    temp;

let numberOfAttempts = document.getElementById("number-of-attempts"), // Получение эллемента попыток по идентификатору number-of-attempts из tg_level4_index.html
    numberOfAttemptsContent = numberOfAttempts.textContent; // Присвоение текстового содержимого переменной

stopwatch.textContent = localStorage.getItem("stopwatchContent");
numberOfAttempts.textContent = localStorage.getItem("numberOfAttemptsContent");
